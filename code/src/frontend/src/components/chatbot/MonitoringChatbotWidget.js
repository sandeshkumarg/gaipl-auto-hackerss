...
import { generateMonitoringDetailedDescription } from '../../utilities/helper';

const MonitoringChatbotWidget = (monitoringdata) => {
  const [collapsed, setCollapsed] = useState(true);
  const [open, setOpen] = useState(false);
  const [messages, setMessages] = useState([
    {
      sender: 'bot',
      text: 'Hello! I am your monitoring assistant. How can I help you today?'
    }
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await fetch("http://localhost:8000/api/monitoringchat", {
        ...
        body: JSON.stringify({ chatid: chatID.current, messages: messagesForBackend , monitoringdata: JSON.stringify(monitoringdata) })
      });
      ...
      setMessages((prev) => [
        ...prev,
        { sender: 'bot', text: "Error: Unable to retrieve a response from our monitoring assistant." }
      ]);
    }
  };
  ...
  return (
    <>
      ...
      >
        ...
        >
          {collapsed ? "Monitoring ChatBot" : "Monitoring Assistant"}
        </div>
...
export default MonitoringChatbotWidget;